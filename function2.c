...
int searchNode(lin_t *head, char *str)
{
	...
	while (current)
	{
		...
		for (i = 0; i < len; i++)
			temp[i] = current->str[i];
		temp[i] = '\0';
		if (_strcmp(str, temp) == 0)
		{
			free(temp);
			return (index);
		}
...
/**
 * generateLinkedList - generates a linked list of env variables
...
/**
 * addNodeAtIndex - adds node at index with string
